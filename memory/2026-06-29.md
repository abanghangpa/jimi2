...
### `09:49 UTC` — `6105d2c` feat: deep analysis prep - add ensemble/sweep/M20/confirmation stats
  - jimi_audit/scripts/deep_analysis_prep.py

### `09:36 UTC` — `ba86731` fix: add m20_filter import to scanner
  - jimi_audit/scripts/scanner.py

### `09:35 UTC` — `3156b50` feat: M20 filter - contrarian block + staleness + level export
  - jimi_audit/scripts/scanner.py

### `09:31 UTC` — `3f9e185` feat: M20 repurpose - contrarian block + staleness filter + level export
  - jimi_audit/src/m20_filter.py

### `09:14 UTC` — `63c2b26` feat: Phase 3 - regime selector uses m10 components to weight strategies
  - jimi_audit/scripts/scanner.py
  - jimi_audit/src/ensemble.py
  - jimi_audit/src/regime_selector.py

### `08:54 UTC` — `417fb8f` feat: Phase 1+2 - sweep-against filter + M14 detail saving
  - jimi_audit/src/sweep_filter.py

### `08:05 UTC` — `4c8eaf2` fix: insert ensemble gate before confirmation layer (em-dash fix)
  - jimi_audit/scripts/scanner.py

### `08:01 UTC` — `8a1dfdf` feat: ensemble gate - 3+ strategies agree + confirmation (target 70.9% WR)
  - jimi_audit/scripts/scanner.py
  - jimi_audit/src/confirmation.py
  - jimi_audit/src/ensemble.py

### `06:40 UTC` — `25ac5a9` feat: confirmation layer - market-confirmed signals with strategy-specific hold windows
  - REPORTING.md
  - jimi_audit/scripts/scanner.py
  - jimi_audit/src/confirmation.py

### `06:09 UTC` — `4a3a7d6` Add source field to individual strategy signals (to_dict)
  - jimi_audit/src/strategies/base.py

### `05:08 UTC` — `ab2eec7` docs: session log — deep analysis findings, framework defense, structural_break diagnosis
  - jimi_audit/reports/session_log_2026-06-29.md

### `04:58 UTC` — `3736e44` fix: add source tracking to main pipeline signals + deep analysis prep v2
  - jimi_audit/scripts/deep_analysis_prep.py
  - jimi_audit/scripts/scanner.py

### `03:34 UTC` — `da04b89` fix: show nearest swept magnet per direction when all swept
  - jimi_audit/scripts/liquidity_reporter.py

### `03:11 UTC` — `5334762` docs: update STRATEGY_ROADMAP - s05/s06/s07/s11 upgrades done
  - jimi_audit/STRATEGY_ROADMAP.md

### `03:10 UTC` — `aa33e7e` feat: ATR-dynamic thresholds for s05/s06/s07 + bidirectional s11
  - jimi_audit/src/strategies/s05_kill_zone.py
  - jimi_audit/src/strategies/s06_liquidity_grab.py
  - jimi_audit/src/strategies/s07_taker_flow.py
  - jimi_audit/src/strategies/s11_cross_asset.py

### `03:00 UTC` — `2ff3640` docs: mark all PENDING_TASKS as done (M72, M73, TradFi pipeline)
  - jimi_audit/PENDING_TASKS.md

### `02:39 UTC` — `416af2d` feat: multi-timeframe magnets with cache support
  - jimi_audit/scripts/liquidity_collector.py
  - jimi_audit/scripts/liquidity_reporter.py
  - jimi_audit/scripts/scanner.py
