...
def find_stop_clusters(df_15m, idx, sr_levels, lookback=96):
    ...
    # Collect all round number candidates
    round_candidates = []
    for r in range(base - round_step * 10, base + round_step * 11, round_step):
        if r <= 0: continue
        dist = abs(r - current_price) / current_price
        if dist < 0.05:
            round_candidates.append(r)

    # To implement confluence, we'll flag if a round number is ALSO a swing level
    for r in round_candidates:
        # Check if this round number is near a recent swing H/L (Confluence!)
        # If it is, we give it a "confluence multiplier"
        confluence_multiplier = 1.0
        
        # Check proximity to recent swing highs (from the swing logic below)
        # We'll do this after calculating swings to avoid multiple passes.
        # For now, we'll just prepare the zone and apply the multiplier later.
...
def fetch_order_book_depth(symbol="ETHUSDT", limit=100):
    ...
    Tries Binance, Coinbase, then Kraken (fallback).
    Returns (bids, asks, source) where source is one of 'binance', 'coinbase', 'kraken', or 'none'.
    """
    ...
    # Try in order: Binance -> Coinbase -> Kraken
    for fetcher in (try_binance, try_coinbase, try_kraken):
        bids, asks, source = fetcher(symbol, limit)
        if bids and asks:
...
def estimate_liquidity_levels(df_15m, idx, sr_levels, oi_usd, ls_ratio,
    ...
    if order_book:
        ...
        # Calculate liquidity density around this price level
        # We'll look for pockets of volume, not just single large orders.
        # Group orders within a small percentage (e.g., 0.1%) of each other.
        # Then sum volume in those clusters and score based on density.
        density_zones = defaultdict(lambda: {"total_volume": 0, "max_qty": 0, "count": 0})
        
        # Define price buckets for density calculation
        price_buckets = np.linspace(current_price * 0.98, current_price * 1.02, 20) # +/- 2% around current price
        
        for price, qty in bids:
            if price < current_price * 0.98 or price > current_price * 1.02: continue # Only consider zones near current price
            bucket_idx = np.searchsorted(price_buckets, price) - 1
            if 0 <= bucket_idx < len(price_buckets) - 1:
                density_zones[bucket_idx]["total_volume"] += qty
                density_zones[bucket_idx]["max_qty"] = max(density_zones[bucket_idx]["max_qty"], qty)
        ...
        for idx, data in density_zones.items():
            ...
            if strength > 15: # Minimum strength to be considered a wall
                ...
                bid_walls_found.append({
                    ...
                    'type': 'BID_WALL',
                    'strength': round(strength, 1),
                    'source': f'Order book ({data["total_volume"]:.0f} ETH clustered)',
                    'cascade_risk': 'LOW',
                    'dist_pct': round((bucket_price - current_price) / current_price * 100, 2),
                })

        for price, qty in asks:
            if price < current_price * 0.98 or price > current_price * 1.02: continue
            bucket_idx = np.searchsorted(price_buckets, price) - 1
            if 0 <= bucket_idx < len(price_buckets) - 1:
                density_zones[bucket_idx]["total_volume"] += qty
                density_zones[bucket_idx]["max_qty"] = max(density_zones[bucket_idx]["max_qty"], qty)
        ...
        # Add found walls to liq_zones
        liq_zones.extend(bid_walls_found)
        liq_zones.extend(ask_walls_found)
