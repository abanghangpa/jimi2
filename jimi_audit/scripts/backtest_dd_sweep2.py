...
RISK_PCT = 0.05; LEVERAGE = 20; TP_PCT = 0.003; SL_PCT = 0.002
HOLD_HOURS = 8; MOM_PERIOD = 12; MOM_THRESHOLD = 0.03
FEE_RATE = 0.0002; BASE_SLIPPAGE = 0.001; DD_STOP = 0.50
MIN_PHASE0 = 0.15; MAX_POSITION_USD = 50000; MAX_CAPITAL = 1_000_000
SEED = 777
...
def get_sig(i,ca,e9,e21,e50,e200,r14,a14,vm):
    if i<200 or e200[i] is None or r14[i] is None: return None
    c=ca[i]['c']; eb=e9[i]>e21[i]>e50[i]; ea=e9[i]<e21[i]<e50[i]
...
def run(ca, cd_hours, si, dd_stop=DD_STOP):
    e9,e21,e50,e200,r14,a14,vm=compute(ca)
    cap=INITIAL_CAPITAL; pk=cap; pos=None; po=None
    cd_until=None; ddt=0; trades=[]; eq=[]; i=si; N=len(ca)
    while i<N:
        ...
        if dd>=dd_stop:
            if cd_hours > 0:
                cd_until=dt+timedelta(hours=cd_hours)
            ddt+=1; eq.append(cap); i+=1; continue
        if cd_until:
            if dt<cd_until: eq.append(cap); i+=1; continue
            cd_until=None
        if po and pos is None:
            sl=slp(cap*LEVERAGE,BASE_SLIPPAGE)
        ...
        if pos is None and po is None:
            sd=get_sig(i,ca,e9,e21,e50,e200,r14,a14,vm)
            md,mv=get_mom(i,ca)
            sig=None
            if sd in('LONG','SHORT'):
                if md==sd: sig=sd
    ...
    return {'cap':round(cap,2),'x':round(cap/INITIAL_CAPITAL,2),'t':len(trades),
            'w':w,'wr':round(w/len(trades)*100,1) if trades else 0,
...
def main():
    ...
    print("="*90)
    print("DD COOLDOWN SWEEP (50% DD threshold, Seed 777, started 2018-11-12)")
    print("="*90)
    print(f"{'Cooldown':>10} | {'Capital':>14} | {'Return':>8} | {'Trades':>6} | {'WR':>6} | {'MaxDD':>6} | {'DDHits':>7} | {'PF':>6}")
    print("-"*90)
    
    cooldowns=[0,2,4,6,8,12,24,48,72,168]
    best=None
    for cd in cooldowns:
        r=run(ca,cd,si)
        label=f"{cd}h" if cd>0 else "OFF"
    ...
    print("-"*90)
    print(f"Best cooldown: {best['cd']}h → ${best['r']['cap']:,.2f} ({best['r']['x']}x)")
    
    print("\n" + "="*90)
    print("DD THRESHOLD SWEEP (Best cooldown from above)")
    print("="*90)
    print(f"{'DD Stop':>10} | {'Capital':>14} | {'Return':>8} | {'Trades':>6} | {'WR':>6} | {'MaxDD':>6} | {'DDHits':>7} | {'PF':>6}")
    print("-"*90)
    
    thresholds=[0.20,0.25,0.30,0.35,0.40,0.45,0.50,0.60,0.70]
    best_t=None
    for t in thresholds:
        r=run(ca,best['cd'],si,dd_stop=t)
        print(f"{t*100:>9.0f}% | ${r['cap']:>12,.2f} | {r['x']:>7.2f}x | {r['t']:>6} | {r['wr']:>5.1f}% | {r['dd']:>5.1f}% | {r['ddt']:>7} | {r['pf']:>6}")
        if best_t is None or r['cap']>best_t['cap']: best_t={'t':t,'r':r}
    
    print("-"*90)
    print(f"Best DD threshold: {best_t['t']*100:.0f}% → ${best_t['r']['cap']:,.2f} ({best_t['r']['x']}x)")
    
    # Combined best
    print(f"\n{'='*90}")
    print(f"OPTIMAL: DD {best_t['t']*100:.0f}% + {best['cd']}h cooldown")
    print(f"Result: ${INITIAL_CAPITAL:,.2f} → ${best_t['r']['cap']:,.2f} ({best_t['r']['x']}x)")
    print(f"Trades: {best_t['r']['t']} ({best_t['r']['w']}W) | WR: {best_t['r']['wr']}% | PF: {best_t['r']['pf']}")
    print(f"{'='*90}")
