...
def scan_signal(df_15m, df_1h, df_2h, df_4h, df_1d, config=None,
    ...
    # M3 hard fail — but not when M20 direct signal or squeeze is active
    # (failed breakouts naturally have poor VWAP scores; squeezes override structure)
    m20_override_active_for_m3 = dir_details.get('m20_override') is not None
    m20_strong_for_m3 = m20_status == 'PASS' and m20_score >= cfg.get('M20_DIRECT_SIGNAL_THRESHOLD', 0.70)
    _squeeze_active_early = squeeze_result['squeeze_status'] == 'TRIGGERED' and squeeze_confirmed
    if m3_status == 'FAIL' and not (m20_override_active_for_m3 and m20_strong_for_m3) and not _squeeze_active_early:
        result['status'] = 'NO_SIGNAL'
        result['reason'] = 'M3 VWAP fail'
        return result

    # ICS check — with bypass paths for event-driven signals
    if ics < effective_floor or ics < threshold:
        # ── Squeeze Bypass ──
        # Squeeze confirmed = volatility compression → expansion. Self-confirming.
        if squeeze_result['squeeze_status'] == 'TRIGGERED' and squeeze_confirmed:
            result['ics_bypassed_by_squeeze'] = True
            print(f"  ⚡ SQUEEZE BYPASS: squeeze confirmed — ICS {ics:.3f} < {threshold:.2f} overridden")
        ...
        # that overrode the direction, allow M20 to generate a signal directly.
            ...
            m20_direct_threshold = cfg.get('M20_DIRECT_SIGNAL_THRESHOLD', 0.70)
    ...
    # ── M14 Sweep Gate ──
    # Bypass for M20 direct signals, squeeze-triggered entries, AND any M20 PASS
    # (failed breakouts are self-confirming events — they don't need sweep confirmation)
...
def main():
    ...
    # ── Multi-Strategy Runner (21 strategies + order flow) ──
    try:
        # Fetch real-time order flow data
        _ob_data = fetch_multi_exchange_ob('ETH/USDT', depth=20)
        _trade_data = fetch_recent_trades('ETH/USDT', 'binance', limit=100)
        _liq_data = fetch_liquidations('ETHUSDT')
        _fr_data = fetch_funding_rates('ETH/USDT:USDT')
        _order_flow_kwargs = {
            'order_flow': _ob_data or {},
            'trade_flow': _trade_data or {},
            'liquidations': _liq_data or {},
            'funding_rates': _fr_data or {},
        }
        result['order_flow'] = {
            'ob_imbalance': _ob_data.get('avg_imbalance') if _ob_data else None,
            'ob_consensus': _ob_data.get('consensus') if _ob_data else None,
            'trade_taker': _trade_data.get('taker_ratio') if _trade_data else None,
            'trade_net_flow': _trade_data.get('net_flow') if _trade_data else None,
            'liq_imbalance': _liq_data.get('liq_imbalance') if _liq_data else None,
